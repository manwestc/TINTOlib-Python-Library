...
class IGTD(AbstractImageMethod):
        ...
        for i in range(data_2.shape[0]):
            ...
            if image_folder is not None:
                if self.problem == "supervised":
                    route = self.__saveSupervised(labels[i], i, image_data[:, :, i], fig, ax)
                    imagesRoutesArr.append(route)
                elif self.problem == "unsupervised" or self.problem == "regression":
                    route = self.__saveRegressionOrUnsupervised(i, image_data[:, :, i], fig, ax)
                    imagesRoutesArr.append(route)
                else:
    ...
    def get_feature_coordinates(self, with_names: bool = False) -> dict:
        """
        Returns a dictionary mapping each original feature index to its (row, column) position
        in the final image layout after transformation. Optionally includes feature names.
        ...
        """
        if not hasattr(self, 'index') or not hasattr(self, 'coordinate') or not hasattr(self, 'feature_names'):
            raise RuntimeError("The transformation has not been fitted yet. Please call `fit()` first.")

        best_order = self.index[self.min_id]
        row_coords, col_coords = self.coordinate

        feature_to_position = {}
        for pos, original_feature_idx in enumerate(best_order):
            if with_names:
                feature_to_position[original_feature_idx] = {
                    'name': self.feature_names[original_feature_idx],
                    'position': (int(row_coords[pos]), int(col_coords[pos]))
                }
            else:
                feature_to_position[original_feature_idx] = (int(row_coords[pos]), int(col_coords[pos]))
