...
class TINTO(AbstractImageMethod):
    ...
    def get_feature_coordinates(self, with_names: bool = False) -> dict:
        ...
        in the final image layout. Optionally includes feature names.
        ...
        """
        if not hasattr(self, 'pos_pixel_caract') or self.feature_names is None:
            raise RuntimeError("The TINTO algorithm has not been fitted yet. Please call `fit()` first.")

        feature_to_position = {}
        for i, (row, col) in enumerate(self.pos_pixel_caract):
            if with_names:
                feature_to_position[i] = {
                    'name': self.feature_names[i],
                    'position': (int(row), int(col))
                }
            else:
                feature_to_position[i] = (int(row), int(col))
